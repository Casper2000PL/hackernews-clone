  ...
  .get(
    ...
    zValidator(
      ...
      paginationSchema.extend({
        includeChildren: z.coerce.boolean().optional(), // use to be `boolean({ coerce: true })` but zod syntax has changed
      }),
    ),
    async (c) => {
      const user = c.get("user");
      ...
      const [postExists] = await db
        .select({ exists: sql`1` }) // clause checks if there is at least one row in the table matching the condition
        .from(postsTable)
        .where(eq(postsTable.id, id))
        .limit(1);
      ...
      if (!count) {
        throw new HTTPException(500, { message: "Failed to count comments" });
      }
